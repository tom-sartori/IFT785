...
class Dsl:
    ...
    def _is_block_definition_verified(self, block_definition: dict) -> bool:
        ...
        if "transaction_fee" in block_definition.keys() and not is_divisible:
            print(f'Error: Block {block_definition["unit"]} Cannot have transaction_fee and not divisible '
                  f'(is_divisible: false). Note that by default is_divisible is true, so you have to set '
                  f'it explicitly to false otherwise. ')
            return False
        ...
        for key in block_definition.keys():
            ...
            if (not isinstance(block_definition[key], attribute_type)
                    and not isinstance(block_definition[key], type(None))):
                print(
                    f'Error: Block type {block_definition} has wrong type for key {key} with value {block_definition[key]}. ')
                return False
